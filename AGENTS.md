...
VS Code file icon theme extension that uses Octicons and Codicons fonts. Two theme variants are generated from a shared data-driven configuration. All source is plain JavaScript (CommonJS) with no bundler.
...
scripts/generate-theme.js          Entry point — iterates package.json iconThemes
  └─ src/themes.js                 generateIconTheme() builds and writes each theme
       ...
       ├─ src/codepoints/{id}.json Font character codes per theme (octicons / codicons)
...
Output: `theme/unfancy-octicons.icon-theme.json` and `theme/unfancy-codicons.icon-theme.json`
...
- Only Octicons and Codicons glyphs
